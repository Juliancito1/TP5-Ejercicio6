...
function setTime() {
  document.getElementById("temporizador").innerHTML = `${String(
    horas.value
  ).padStart(2, "0")}:${String(minutos.value).padStart(2, "0")}:${String(
    segundos.value
  ).padStart(2, "0")}`;
...
function reiniciar() {
  ...
  document.getElementById("temporizador").innerHTML = `${String(
    reducirHoras
  ).padStart(2, "0")}:${String(reducirMinutos).padStart(2, "0")}:${String(
    reducirSegundos
  ).padStart(2, "0")}`;
  btnReiniciar.disabled = true;
}
